...
export class ReceivingNotesService {
  ...
  getAll(params?: {
    ...
    return this.apiService.get<PaginatedReceivingNoteResponse>('ReceivingNotes', params);
  }

  getById(id: string): Observable<ApiResponseModel<ReceivingNoteDetailDto>> {
    return this.apiService.get<ReceivingNoteDetailDto>(`ReceivingNotes/${id}`);
  }

  create(data: CreateReceivingNoteCommand): Observable<ApiResponseModel<string>> {
    return this.apiService.post<string>('ReceivingNotes', data);
  }

  approve(id: string, data: ApproveReceivingNoteCommand): Observable<ApiResponseModel<any>> {
    return this.apiService.post<any>(`ReceivingNotes/${id}/approve`, data);
  }
}
