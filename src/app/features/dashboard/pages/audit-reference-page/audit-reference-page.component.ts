...
export class AuditReferencePageComponent {
  ...
  workflowNotifications = signal<NotificationMessage[]>([]);
  ...
  private loadWorkflowNotifications(): void {
    ...
    this.workflowNotifications.set(
      this.workflowService.getNotificationsForUser(user.id, role)
    );
  }
  ...
  wfUnreadCount(): number {
    return this.workflowNotifications().filter(n => !n.isRead).length;
  }
