      ...
      <header class="dashboard-header">
        <div class="header-content">
          <div class="header-text">
            ...
            <p class="header-description">Manage and oversee all service requests across the organization</p>
          </div>
                  ...
                  @for (request of recentRequests(); track request.id) {
                    <tr class="request-row">
                      ...
                      <td class="items-info">
                        ...
                        <div class="items-preview">{{ request.items[0]?.name }}@if (request.items.length > 1) { +{{ request.items.length - 1 }} more }</div>
                      </td>
                      <td class="priority">
            ...
            @for (notification of workflowNotifications().slice(0, 5); track notification.id) {
              <div class="notification-item" [class.unread]="!notification.isRead">
                <div class="notification-icon" [class]="'notification-icon--' + notification.type">
                  <i class="bi" [class]="notification.type === 'success' ? 'bi-check-circle' : 
                                       notification.type === 'error' ? 'bi-x-circle' : 
                                       notification.type === 'warning' ? 'bi-exclamation-triangle' : 'bi-info-circle'"></i>
                </div>
                <div class="notification-content">
                  <h4 class="notification-title">{{ notification.title }}</h4>
                  <p class="notification-message">{{ notification.message }}</p>
                  <time class="notification-time">{{ notification.createdDate | date:'short' }}</time>
                </div>
                @if (notification.actionRequired) {
                  <div class="notification-action">
                    <button class="btn btn--small btn--primary" type="button">Action Required</button>
                  </div>
                }
              </div>
            } @empty {
...
export class AdminDashboardComponent implements OnInit, OnDestroy {
  ...
  readonly summaryCards = computed<AdminSummaryCard[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: requests.filter(r => r.status === 'Completed' && this.isToday(r.completedDate)).length,
  ...
  private setupWorkflowSubscriptions(): void {
    ...
    const notificationSub = this.workflowService.getNotificationUpdates().subscribe(notification => {
  ...
  private loadWorkflowData(): void {
    ...
    const notifications = this.workflowService.getNotificationsForUser(this.currentAdminId, 'Admin');
